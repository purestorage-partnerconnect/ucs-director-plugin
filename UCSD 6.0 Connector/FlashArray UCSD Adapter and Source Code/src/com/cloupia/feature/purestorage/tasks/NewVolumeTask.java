...
import com.cloupia.model.cIM.CMDBIntegrationSettings;
import com.cloupia.model.cIM.ChangeRecord;
import com.cloupia.model.cIM.ServiceRequestAssetChangeEntry;
        ...
        if(destroyVolumeTaskFlag == null)
         {
            ...
            /*ChangeRecord rec = new ChangeRecord();
                ...
                rec.setResourceType("FlashArray VOLUME");
                rec.setDescription("Volume Name = "+ volumePreName+" , Volume Size = "+volumeSizeNumber+volumeSizeUnit);
                rec.setChangeType(1);
                rec.setChangeByUser(context.getUserId());
                rec.setChangedResourceName(volumePreName);
                rec.setAdditionalInfo("Volume Name = "+ volumePreName+" , Volume Size = "+volumeSizeNumber+volumeSizeUnit);
             ...
             ObjStore<ChangeRecord> catStore = ObjStoreHelper.getStore(ChangeRecord.class);
               ...
               /* ServiceRequestAssetChangeEntry e = new ServiceRequestAssetChangeEntry();
                e.setRequestId(context.getSrId());
                e.setChangeTime(System.currentTimeMillis());
                if (context.getSR() != null) {
                  e.setGroupId(context.getSR().getGroupId());
                }
                e.setCutomActionId(context.getActionDef().getId());
                e.setCustomActionEntryId(context.getConfigEntry().getConfigEntryId());
                e.setTaskName(context.getConfigEntry().getTaskName());
                
                e.setChangeType(1);
                e.setChangeDescription("Volume Name = "+ volumePreName+" , Volume Size = "+volumeSizeNumber+volumeSizeUnit);
                e.setAssetType("FlashArray VOLUME");
                e.setAssetId(connVol);
                e.setAssetLabel(volumePreName);
              ObjStore<ServiceRequestAssetChangeEntry> store = ObjStoreHelper.getStore(ServiceRequestAssetChangeEntry.class);
                  store.insert(e);
            */
